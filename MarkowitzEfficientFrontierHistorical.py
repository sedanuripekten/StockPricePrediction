...
#Reading the historical_full_df we created in FBProphetForecast.py file for comparison of performance between real data and forecasted data
hist_df = pd.read_csv("//stock_full_historical.csv")
...
def prepare_pivot_data(dataframe):
    ...
    return dataframe
hist_df = prepare_pivot_data(hist_df)
hist_df.head()
hist_df.shape
...
#TICKERS
tickers = hist_df.columns.tolist()
...
def efficient_frontier_plot(dataframe):
    ...
    plt.tight_layout()
    plt.show()
efficient_frontier_plot(hist_df)
